...
async function fetchRSSEpisodes() {
  ...
  try {
    ...
    while ((match = itemRegex.exec(xml)) !== null) {
      ...
      // Aggressive HTML stripping — handles CDATA, encoded entities, nested tags
      description = description
        .replace(/<!\[CDATA\[/g, '').replace(/\]\]>/g, '')  // Strip CDATA wrappers
        .replace(/&lt;/g, '<').replace(/&gt;/g, '>')        // Decode entities FIRST
        .replace(/&amp;/g, '&').replace(/&quot;/g, '"').replace(/&#39;/g, "'")
        .replace(/<[^>]*>/g, '')                             // Strip all HTML tags
        .replace(/&nbsp;/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
      // Take only the first meaningful paragraph (before any "In this episode" boilerplate)
